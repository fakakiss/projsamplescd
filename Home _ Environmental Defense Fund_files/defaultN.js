...
edf.general = function () {
	...
	return {
		ready: function () {
			...
			$jq('.radioSubGroup p').append('<img class="radioGroupLeft" src="/sites/all/themes/edf/images/radioGroupLeft.gif"/><img class="radioGroupRight" src="/sites/all/themes/edf/images/radioGroupRight.gif"/>');
			$jq('.colPad hr:last-child').css({ 'margin-bottom': "25px" });
			$jq('#pageIntro').parent('.row').css({ 'margin-bottom': "0" });
			$jq('#breakingNews').append('<div class="close"><img src="/sites/all/themes/edf/images/closelabel.png" /></div>').click(function () {
		...
		initTabbedAreas: function () {
			$jq('.tabbed').each(function () {
				...
				container.children('h3').each(function (index) {
					...
					tabs.append($jq('<li title="tab' + index + '">' + $jq(this).text() + '<img src="/sites/all/themes/edf/images/tab.gif" /></li>').click(
                    function () {
		...
		initTabbed2Areas: function () {
			$jq('.tabbed2').each(function () {
				...
				container.children('h3').each(function (index) {
					...
					tabs.append($jq('<li title="tab2_' + index + '">' + $jq(this).text() + '<img src="/sites/all/themes/edf/images/tab.gif" /></li>').click(
                    function () {
                    	container.find('.current').removeClass("current");
                    	$jq(this).addClass("current");
                    	container.find('.tab').css({ 'display': 'none' });
    ...
    return {
        ...
        init: function () {
            ...
            $jq('.flyout .pageWidth').each(function () {
                ...
                if (shite) $jq(this).append($jq('<div class="close"><img src="/sites/all/themes/edf/images/closelabel.png" /></div>').click(function () { that.closeFlyouts(); }));
