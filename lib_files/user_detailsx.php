<?php
	...
	if(mysqli_num_rows($rst_user_rights)>0)
	{$row_user_rights = mysqli_fetch_array($rst_user_rights) or die(mysqli_error($conn));}
	
	if (empty($_user_rights))	{$_user_rights     ="";}
	else {$_user_rights = $row_user_rights['mainpriv'];}
...
<?php
	...
	//include("db.class.php");
	 ...
	 //$conn          = mysqli_connect($mysql_serv,$mysql_user,$mysql_pass) or die(mysqli_error($conn));
	                  //mysqli_select_db($conn,$mysqli_db) or die(mysqli_error($conn));
	...
	//@$q_user_rights             	 	= "select * from users where username='$_sess_username'";
	//@$rst_user_rights           		= mysqli_query($conn,@$q_user_rights) or die(mysqli_error());
	//if(mysqli_num_rows(@$rst_user_rights)>0){@$row_user_rights = mysqli_fetch_array(@$rst_user_rights) or die(mysqli_error());}
	
	//@$_user_rights = $row_user_rights['mainpriv'];
