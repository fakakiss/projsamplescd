<?php
	...
	if ($_sess_username=='admin') {$q = "select * from users order by username";  $rst = mysqli_query($conn,$q) or die(mysql_error());}   
	else                          {$q = "select * from errors where name='cass'"; $rst = mysqli_query($conn,$q) or die(mysql_error());} 
...
<head><title><?php echo $clientname;?> | System Users</title></head>
...
<?php cmshead($privite_site_width,$privite_nav_width,$_sess_username,$_user_rights)?>
 ...
 <img src="../../uploads/images/<?php echo $row['picture']?>" width="<?php echo "$tn_width";?>" height="<?php echo "$tn_height";?>" class="cms_image_border">
  ...
  <?php }}else{?>
  <tr><td class="error">You don't have aurthorization for this section of the system.</td></tr>
