<?php
	...
	if ($search=="") 
	{
	$err_msg="You did not enter a search string.";
	$q = "select * from users where username=''";
	$rst = mysql_query($q) or die(mysql_error());
	}
	else
	{
	//$q = "SELECT username,CONCAT(firstname, SPACE(1), lastname) as fullname, updated FROM users ORDER BY username";
	$q = "select * from users where username like '%$search%' order by username asc";
	$rst = mysql_query($q) or die(mysql_error());
...
<?php cmshead($privite_site_width,$privite_nav_width,$_sess_username,$_user_rights)?>
 ...
 <img src="../../uploads/images/<?php echo $row['picture']?>" width="<?php echo "$tn_width";?>" height="<?php echo "$tn_height";?>" class="cms_image_border">
  ...
  <?php }}else{?>
  <tr><td class="error"><?php echo "";?></td></tr> 
  <?php }?>
</table>
                              </div>
							   </div>
