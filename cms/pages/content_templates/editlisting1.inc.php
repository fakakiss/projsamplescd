<?php top_module($manager,$add,$view,$viewcat,$addcat,$_user_rights,$_sess_username);?>
	...
	<?php if(!empty($err_msg)){?>
    	<div style="padding-bottom:10px;padding-top:10px; color:#FF0000" class="cms_text">Listing Title Required</div>
	<?php }?>
									
	<form enctype="multipart/form-data" action="<?php echo $the_page."?mpid=".$mpid."&spid=".$spid."&lpid=".$lpid;?>" method="post">
			...
			<div style="padding-bottom:5px">
			  <table border="0" cellspacing="0" cellpadding="0" class="cms_text">
			    ...
			    <tr>
                  ...
                  <input type="hidden" value="OK" name="send"><input name="submit" type="submit" value="<?php echo "$submit_bottom_go";?>">
			    ...
			    <tr>
			      ...
			      <td><?php if($show_cap_img_select==0){?>
	...
	?>
			    ...
			    <?php if($show_opt_show==1){?>
			    	<tr>
			      		<td valign="top" <?php if($bold_opt_show==1){?>class="bold"<?php }?>><?php echo "$opt_show";?></td>
			      			...
			      			<td valign="top">
                            	<input name="showitem" type="checkbox" value="1" <?php if (($row['showitem'])==1) {echo "checked";} ?> />
                            </td>
               ...
               <!--Category where id is present -->
               <?php if (!empty($mpid)){?>
                    <tr><td <?php if($bold_cat==1){?>class="bold"<?php }?>><?php echo "$cat";?></td> 							                                              
                        <td>
                            <?php 
                                $q_pagedtls       	= "select * from pagecategories where id='$mpid'";
                                ...
                                echo $mpid." ".$row_pagesdtls['caption'];
                                echo $pagename;
                            ?>
			...
			<?php }?>
                ...
                <!--Sub Category Field not empty spid -->
                    <?php if (!empty($spid)){?>
						<tr><td <?php if($bold_cat==1){?>class="bold"<?php }?>><?php echo "$subcat";?></td> 							                                              
          					<td>
		  						<?php 
									$q_subpagedtls       	= "select * from pages where category='$spid'";
    								$rst_subpagedtls     	= mysql_query($q_subpagedtls) or die(mysql_error());
									if(mysql_num_rows($rst_subpagedtls)>0)
									{$row_subpagedtls = mysql_fetch_array($rst_subpagedtls) or die(mysql_error());} 	
									$subpagename			=$row_subpagesdtls['caption'];				
									echo $spid." ";
									echo "$subpagename";
								?>
                                <span class="error"><?php echo "$cat_help";?></span>	 
					...
					<?php }?>
                 	...
                 	<!--Sub Category Field  where spid is 0-->
                    <?php if (empty($spid)){?>
                  		<tr>
                      		<td valign="top" <?php if($bold_cat==1){?>class="bold"<?php }?>><?php echo "$subcat";?></td>
                      		<td valign="top">
                            	<select name="subcategory" class="form" style="width:<?php echo "$cat_width";?> ">
                        			<option value="0">Unsubcategorized
                          				<?php
    										$rstsubcat = mysql_query($qsubcat) or die(mysql_error());
    										while($rowsubcat = mysql_fetch_array($rstsubcat))
												{
													if($row['subcategory']==$rowsubcat[0])
													{
														echo "<OPTION VALUE=\"$rowsubcat[0]\" SELECTED>$rowsubcat[caption]";
													}
													else
													{
														echo "<OPTION VALUE=\"$rowsubcat[0]\">$rowsubcat[caption]";
													}
												}
    										?>
                        			</option>
 ...
 <img src="../../uploads/images/<?php echo $row['img5']?>" width="<?php echo "$tn_width";?>" height="<?php echo "$tn_height";?>" class="cms_image_border"> <a href="<?php echo $the_page;?>?id=<?php echo $row[0]?>&&del=img5"><img src="../cms_images/del.jpg" border="0"></a>
	...
	<?php }?>
					...
					?>
                                                        ...
                                                        <tr><td><?php echo "$imagelink_caption5";?>
</td></tr>
                                                        <tr><td>
