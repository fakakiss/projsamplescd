<?php
	...
	$q = "select name from docs where name='$mysql_id'";
	$rst = mysql_query($q) or die(mysql_error());
	$row = mysql_fetch_array($rst);
	$delfile = $row[0];
	
	$quse = "select id from pages where doc='$delfile' ";
	$rstuse = mysql_query($quse) or die(mysql_error());
	
	
	if(mysql_num_rows($rstuse) > 0  )
	{
		$err_msg = "The Document is being used in a <strong><u>module</u></strong> and cannot be deleted.<br>";
	...
	else
	{
		...
		if(file_exists("../../uploads/docs/$delfile")){unlink("../../uploads/docs/$delfile");}
		
		$qdel = "delete from docs where name='$mysql_id'";
