<?php
    ...
    $q = "select * from video order by name asc";
    $rst = mysqli_query($conn,$q) or die(mysqli_error($conn));
	
	if($play==1) 
	{ 
	$q1 = "select * from video where name='$soundname'";
		...
		<div style="padding:10px">
			...
			<?php top_module($manager,$add,$view,$viewcat,$addcat,$_sess_username,$_user_rights);?>
				...
				<div  style="padding-top:10px;">
					...
					<embed autostart="false"  height="200px" width="200px"  src="../../uploads/video/<?php echo stripslashes($row1['name'])?>"></embed>
			        </div>
					<?php }?>
					...
					<table  border="0"  cellpadding="0" cellspacing="0" class="cms_text" >
                		...
                		<?php if(mysqli_num_rows($rst) > 0){while($row = mysqli_fetch_array($rst)){?>
						   	...
						   	<?php if(!empty($row['name']) && file_exists("../../uploads/video/".$row['name'])){ ?>		
			  				   			...
			  				   			<table border="0" cellspacing="0" cellpadding="0"   class="cms_text">
                  							<tr>
          										<td width=60px valign="top" height="110px">		  
		                                         	<?php if(($row['type'])=="video/x-ms-asf") { ?><a href="../../uploads/video/<?php echo $row[0]?>"><img src="../cms_images/wmavid.jpg" class="cms_image_border"></a><?php }?>  														  													  
		  											<?php if(($row['type'])=="video/x-ms-wmv") { ?><a href="../../uploads/video/<?php echo $row[0]?>"><img src="../cms_images/wmavid.jpg" class="cms_image_border"></a><?php }?>		  													  									  
		  											<?php if(($row['type'])=="application/octet-stream") { ?><a href="../../uploads/video/<?php echo $row[0]?>"><img src="../cms_images/wmavid.jpg" class="cms_image_border"></a><?php }?> 		  													  																																				
													<?php if(($row['type'])=="video/avi") { ?><a href="../../uploads/video/<?php echo $row[0]?>"><img src="../cms_images/wmavid.jpg" class="cms_image_border"></a><?php }?> 			  																					
													<?php if(($row['type'])=="video/mpeg") {?><a href="../../uploads/video/<?php echo $row[0]?>"><img src="../cms_images/wmavid.jpg" class="cms_image_border"></a><?php }?>  
		    									</td> 
                   								<td valign="top" > 
            				...
            				<?php }}else{?><tr><td class="error">No Videos.</td></tr><?php }?>
