<?php
	...
	$q = "select name from video where name='$mysql_id'";
	$rst = mysql_query($q) or die(mysql_error());
	$row = mysql_fetch_array($rst);
	$delfile = $row[0];
	
	$quse = "select id from pages where(video='$delfile' )";
	$rstuse = mysql_query($quse) or die(mysql_error());
	
	
	if(mysql_num_rows($rstuse) > 0  )
	{
		$err_msg = "This sound file is being used in a <strong><u>module</u></strong> and cannot be deleted.<br>";
		$err_msg = $err_msg . "You will be redirected in 5 seconds. Please stand by.";
		echo "<META HTTP-EQUIV=\"Refresh\" CONTENT=\"5;URL=index.php\">";
	}
	else
	{
		if(file_exists("../../uploads/video/$delfile")){unlink("../../uploads/video/$delfile");}
		$qdel = "delete from video where name='$mysql_id'";
