<?php
        ...
        $q = "select * from audio where name='$mysql_id'";
        $rst = mysql_query($q) or die(mysql_error());
        $row = mysql_fetch_array($rst) or die(mysql_error());
        if(isset($send) && ($send=="OK"))
	   {
		$q_update = "update audio set caption='$caption',description='$description',updated=NOW() where name='$mysql_id'";
...
<?php if(($row['type'])=="audio/mpeg") { ?> 
		  												<img src="../cms_images/mp3.gif" class="cms_image_border">
		  											<?php }?>
		  
		  											<?php if(($row['type'])=="audio/wav") { ?> 
		  												<img src="../cms_images/wav.gif" class="cms_image_border">
		  											<?php }?>
		  
		  											<?php if(($row['type'])=="audio/x-ms-wma") { ?> 
		  												<img src="../cms_images/wma.jpg" class="cms_image_border">
		  											<?php }?>
