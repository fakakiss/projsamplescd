<?php
        ...
        $q = "select * from packages where name='$mysql_id'";
        $rst = mysql_query($q) or die(mysql_error());
        $row = mysql_fetch_array($rst) or die(mysql_error());
		
        if(isset($send) && ($send=="OK"))
	   		{
				$q_update = "update packages set caption='$caption',description='$description',updated=NOW() where name='$mysql_id'";
...
<head><title><?php echo $clientname;?> | Packages | Edit </title></head>
<body>
...
<table   class="cms_text">
		                    ...
		                    <tr><td>Caption:</td>
                            <td><?php echo ucfirst($row['caption'])?></td>                 
                            </tr>
                            <?php } ?>
...
</div><img src="../../uploads/images/<?php echo $row['name']?>" width="<?php echo "$tn_width";?>" height="<?php echo "$tn_height";?>"  class="cms_image_border">
</div>
