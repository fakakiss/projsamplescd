				...
				if ($err_msgf=="") {include('incs/register_img_upload1.inc.php');}
		 		...
		 		if ($err_msgf=="")
					{	
						...
						$qinsert 			= "insert into feedback_form values(NULL,'$fbid','$location','$firstname','$surname','$image_name','$email','$cell','$passport','1','$dob','$s1','0','0','0','0',md5('$pass1'),'0','$acceptedn','$acceptedt','$dob','$gender','$atype',now())";
						
						$rstinsert 			= mysqli_query($conn,$qinsert) or die(mysql_error());
...
?>
	...
	//if($data['page']['liked'] == 1){$fan = "yes";}else {$fan = "no";}
			...
			$rstfbidpreznt  = mysqli_query($conn,$qfbidpreznt) or die(mysql_error($conn));
			if(mysqli_num_rows($rstfbidpreznt)>0)
				{
					$rowfbidpreznt=mysqli_fetch_array($rstfbidpreznt) or die(mysql_error($conn));
					$played_promo=1;
				}
...
<?php if (  (!($played_promo==1) )  ) {?>
	...
	<div id="user-info"></div>
		...
		<div class="about-welcome-inner">
            ...
            <p>
                Complete the form below to get your WebKings Member Passport and stand a chance of winning a Scholarship Grant for 1 year or 6 months of Web/Mobile Development Training and Placement. Also amazing trips to one of the following Countries i.e. South Africa,France,Spain,USA,Canada for Student/Technology Conferences with $1000 in spending money if you rank in the top 5 of your class at the end of the practical training period.
            </p>
			...
			<form class="form-horizontal" enctype="multipart/form-data" action="index.php?page_ren=101" method="post">
...
<table width="100%" border="0" cellspacing="0" cellpadding="0"> -->
 ...
 <!-- <tr>
    <td><span style="color:<?php echo $red_dob?>"><strong>Date of Birth:</strong></span> 
  ...
  <tr>
    ...
    <select name="location" id="edit-state" class="form-select">
			            						<option value="Accra [Ghana]">Accra [Ghana] 
                									<?php
                    									$rstlocation=mysqli_query($conn,$qlocations) or die(mysql_error($conn));
...
<?php if($postSuccess==1){?>
		...
		function send_invite($toEmail,$toName,$fromEmail,$fromName)
			{
				...
				$transport = Swift_SmtpTransport::newInstance('mail.webkings.co.za', 25)
					->setUsername('scholarships@webkings.co.za')
				...
				Step 5 Orientation and Start of Class [Gaurenteed Job Placement After 3 to 6 months of Practical Training]
	...
	<?php
        ...
        'picture' => $img,
        'description'=>"130 Students to Selected from around South Africa & Ghana  for a 1 year Training and Placement Project in Web & Mobile APP & Content Development.
